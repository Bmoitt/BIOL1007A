...
p<- ggplot(data=dryadData, # this says that you would like a plot or graph to be made
       aes(x=interaction(wetland, treatment), # the x values will be the wetland and treatment columns
           y=count.total.adults, fill=factor(year))) + geom_bar(position="dodge", stat="identity", color="black") + # the y values will be the total adult count. geom_bar is requesting a bar graph where the position is dodge which means _____ and the stats are _______ while the color for the bars are black
...
legend.position="top",  legend.title= element_blank(), axis.title.y = element_text(size=12, face="bold", colour = "black"), strip.text.y = element_text(size = 10, face="bold", colour = "black")) + #makes the legend go to the top with legend.position. for the strip text, the siz was adjusted to be 10, the font size was made to be bold, and the print color was made to b black
